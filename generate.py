...
def load_tokenizer(config, model_type: str = "gpt2", tokenizer_path: Optional[str] = None):
    """加载分词器"""
    ...
    for i in range(num_return_sequences):
        ...
        if isinstance(tokenizer, GPT2Tokenizer):
            try:
                ...
                generated_text = "".join(
                    [chr((tid - 10) % 256) for tid in gen_ids if tid >= 10]
                )
        else:
            generated_text = tokenizer.decode(gen_ids, skip_special_tokens=True)
...
def main():
    ...
    # 加载分词器
    tokenizer = load_tokenizer(config, args.model_type, args.tokenizer_path)
