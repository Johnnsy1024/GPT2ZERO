...
def print_model_summary(model: torch.nn.Module):
    ...
    for name, param in model.named_parameters():
        print(f"{name:50} {str(param.shape):30} {param.numel():10,} {'可训练' if param.requires_grad else '冻结'}")
...
def calculate_model_size(model: torch.nn.Module, precision_bits: int = 32) -> Dict[str, float]:
    """计算模型大小
...
def convert_model_to_onnx(model, dummy_input, onnx_path, input_names=None, output_names=None):
    """将PyTorch模型转换为ONNX格式
