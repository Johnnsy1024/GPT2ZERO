    ...
    def __getitem__(self, idx):
        ...
        if isinstance(self.tokenizer, GPT2Tokenizer):
            try:
                ...
                encoded = [self.tokenizer.bos_token_id] + [ord(c) % 1000 + 10 for c in text] + [self.tokenizer.eos_token_id]
        else:
            encoded = self.tokenizer.encode(text, add_special_tokens=True)
        ...
        labels = [token_id if mask == 1 else -100 for token_id, mask in zip(encoded, attention_mask)]
...
def save_checkpoint(model, optimizer, epoch, loss, save_path):
    ...
    checkpoint = {
        ...
        'loss': loss,
        'config': model.config.to_dict() if hasattr(model.config, 'to_dict') else model.config.__dict__
    }
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="./data", help="数据路径（文件或目录）")
    parser.add_argument("--model_type", type=str, default="gpt2", choices=["gpt2", "gpt2-medium", "gpt2-large", "gpt2-xl"], help="模型类型")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 训练
        train_loss = train_epoch(model, dataloader, optimizer, device, epoch + 1)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(example_file):
        with open(example_file, 'w', encoding='utf-8') as f:
            f.write("""GPT-2是一个基于Transformer架构的语言模型。
